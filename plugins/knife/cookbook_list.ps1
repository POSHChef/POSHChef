...
function cookbook_list {
	...
	.DESCRIPTION
		...
		If the plugin detects that a result is expected then it will return an object
		instead of outputting it to the screen

	.EXAMPLE

		PS C:\> Invoke-POSHKnife cookbook list

		List the cookbooks on the server

	.EXAMPLE

		PS C:\> $list = Invoke-POSHKnife cookbook list

		Will result in a hashtable of all the cookbooks known to the chef server.
		The key of the hashtable is the name of the cookbook and the value is the HREF to that
		cookbook on the chef server.
