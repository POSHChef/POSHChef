...
# Import localized data if the file exists
if (Test-Path -Path POSHChef_RegistryResource.strings.psd1) {
	Import-LocalizedData LocalizedData -filename POSHChef_RegistryResource.strings.psd1
}
...
FUNCTION Set-TargetResource
{
	...
	(
		...
		[System.String]
		# Specifies the path to the notifications directorry
		$NotifiesServicePath = [String]::Empty,
...
FUNCTION SetupProvider
{
    ...
    if (!$KeyName.Value.ToString().Contains(":"))
    {
        ...
        {
            $KeyName.Value = $KeyName.Value.ToString().ToLowerInvariant().Replace("hkey_users","HKUS:")
        }
        elseif ($KeyName.Value.ToString().StartsWith("hkey_current_config","OrdinalIgnoreCase"))
        {
            $KeyName.Value = $KeyName.Value.ToString().ToLowerInvariant().Replace("hkey_current_config","HKCC:")
        }
        elseif ($KeyName.Value.ToString().StartsWith("hkey_classes_root","OrdinalIgnoreCase"))
        {
            $KeyName.Value = $KeyName.Value.ToString().ToLowerInvariant().Replace("hkey_classes_root","HKCR:")
        }
        elseif ($KeyName.Value.ToString().StartsWith("hkey_local_machine","OrdinalIgnoreCase"))
        {
            $KeyName.Value = $KeyName.Value.ToString().ToLowerInvariant().Replace("hkey_local_machine","HKLM:")
        }
        elseif ($KeyName.Value.ToString().StartsWith("hkey_current_user","OrdinalIgnoreCase"))
        {
            $KeyName.Value = $KeyName.Value.ToString().ToLowerInvariant().Replace("hkey_current_user","HKCU:")
        }
        else
        {
